...
CP2K is a common DFT backend for generating NEP training labels, especially
for large systems where linear-scaling methods are advantageous.

- **NEP labeling**: use `dft-cp2k/static` for single-point energy+force+stress.
  Recommended: `EPS_SCF 1e-7` or tighter.
- **Conversion**: convert CP2K output to NEP extxyz via
  `GPUMD/tools/Format_Conversion/cp2k2xyz` or `dpdata-cli`:
  ```bash
  uvx dpdata cp2k_output -i cp2k/output -O train.xyz -o extxyz
  ```
- **Stress convention**: CP2K reports stress in GPa by default. Verify
  virial sign and units after conversion.
