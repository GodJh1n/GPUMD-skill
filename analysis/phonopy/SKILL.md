...
GPUMD has its own built-in harmonic phonon capability via `compute_phonon`,
which directly outputs phonon dispersions and DOS without phonopy. Use
phonopy when:

- you need finite-displacement phonons with a DFT force backend (not NEP)
- you need thermal properties (free energy, heat capacity, entropy)
- you want to cross-validate GPUMD `compute_phonon` results
- you need Phono3py for anharmonic / thermal conductivity analysis

GPUMD force-constant output can also serve as input for phonopy-based
post-processing when the force provider is a NEP potential evaluated
through GPUMD or ASE.
